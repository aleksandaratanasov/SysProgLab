...
class ObjectContainer {
  ...
  ObjectContainer<type>* next; // Next container in the chain
  unsigned int count;
...
type* ObjectContainer<type>::newObject() {
    if (count < inititalSize) {   // Check if there is still space left
        return &(data[count++]);  // and return reference
	}

    if (next == 0) { // If next is not pointing anywhere, create a new container in the chain
